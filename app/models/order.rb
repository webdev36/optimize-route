class Order
  ...
  def self.vehicle_data
    data = []
    Order.all[1..-1].each do |order|
      location = {
        "id": order[:delivery_address],
        "name": order[:delivery_address],
        "address": {
          "location_id": order[:delivery_address],
  ...
  def self.get_job_id(data)
    response_data = {}
    resp = {}
    RestClient.post("https://graphhopper.com/api/1/vrp/optimize?key=6c99d61c-7cfd-4075-bd85-5a110ee48874", data.to_json, :content_type => :json, :accept => :json){ |response, request, result, &block|
      case response.code
      when 200
        p "It worked !"
  ...
  def self.get_optimized_routes(job_id)
    response_data = {}
    url = "https://graphhopper.com/api/1/vrp/solution/#{job_id}?key=6c99d61c-7cfd-4075-bd85-5a110ee48874"
